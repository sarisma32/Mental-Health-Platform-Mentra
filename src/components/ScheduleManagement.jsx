...
const ScheduleManagement = () => {
  ...
  const handleAddSchedule = async (e) => {
    ...
    // If today, enforce start time must be after current time
    const now = new Date();
    const todayStr = `${now.getFullYear()}-${(now.getMonth()+1).toString().padStart(2,'0')}-${now.getDate().toString().padStart(2,'0')}`;
    if (formData.scheduleDate === todayStr) {
      const currentHHMM = `${now.getHours().toString().padStart(2,'0')}:${now.getMinutes().toString().padStart(2,'0')}`;
      if (formData.startTime <= currentHHMM) {
        showNotification(`For today, start time must be after current time (${currentHHMM})`, 'error');
        return;
      }
    }
      ...
      <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
        <div className="flex justify-between items-center">
          ...
          <button
            ...
            className="bg-[#4A7C59] hover:bg-[#3d6b4a] text-white px-6 py-3 rounded-lg font-semibold transition-colors flex items-center space-x-2"
          >
      ...
      {showAddForm && (
        <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
          ...
          <form onSubmit={handleAddSchedule} className="space-y-4">
            {/* Today's time restriction hint */}
            {(() => {
              const now = new Date();
              const todayStr = `${now.getFullYear()}-${(now.getMonth()+1).toString().padStart(2,'0')}-${now.getDate().toString().padStart(2,'0')}`;
              const isToday = formData.scheduleDate === todayStr;
                ...
                <>
                  {isToday && (
                    <div className="bg-amber-50 border border-amber-200 rounded-lg px-4 py-2 text-sm text-amber-800 flex items-center gap-2">
                      ...
                      Today selected — start time must be after <strong className="ml-1">{minTime}</strong>
                    </div>
                  )}
                ...
                <div key={date} className="border border-gray-200 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-3">
                    <h4 className="font-semibold text-gray-900 flex items-center">
                      <svg className="w-5 h-5 text-[#4A7C59] mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                      </svg>
                      {formatDate(date)}
                    </h4>
