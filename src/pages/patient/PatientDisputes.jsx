...
const PatientDisputes = ({ patientId }) => {
  const [disputes, setDisputes] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    if (patientId) fetchDisputes();
  }, [patientId]);
  ...
  const fetchDisputes = async () => {
    try {
      ...
      const res = await fetch(buildApiUrl(`${API_ENDPOINTS.PATIENT_DISPUTES}/${patientId}`), {
  ...
  return (
    <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h3 className="text-lg font-semibold text-gray-800">My Disputes</h3>
          <p className="text-sm text-gray-500 mt-0.5">Track the status of your raised disputes</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="text-sm text-gray-400 mt-1">You can raise a dispute from a completed appointment.</p>
        </div>
      ) : (
          ...
          {disputes.map(dispute => {
            ...
            return (
              <div key={dispute.id} className="border border-gray-200 rounded-xl overflow-hidden">
                <div className="p-4">
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      ...
                      <p className="text-sm text-gray-500">Dr. {dispute.doctor_name} — {dispute.doctor_specialization}</p>
                    </div>
                ...
                {/* Admin response */}
                {dispute.admin_response && (dispute.status === 'resolved' || dispute.status === 'rejected') && (
                ...
                {dispute.status === 'under_review' && (
                  <div className="px-4 py-3 border-t bg-blue-50 border-blue-100">
                    <p className="text-xs text-blue-700 flex items-center gap-1">
                      ...
                      Your dispute is currently being reviewed by our admin team.
                    </p>
                  </div>
                )}
              </div>
...
export default PatientDisputes;
