  ...
  return (
    <div className="space-y-6">
          ...
          <div className="relative">
            <div className="w-24 h-24 bg-white/20 rounded-full flex items-center justify-center backdrop-blur-sm border-4 border-white/30 overflow-hidden">
              {doctor.profile_photo ? (
                <img src={`http://localhost:5002/${doctor.profile_photo}?t=${photoTimestamp}`} alt={doctor.full_name} className="w-full h-full object-cover" key={photoTimestamp} />
              ) : (
      ...
      <form onSubmit={handleProfileSave} className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              ...
              {!isEditing && (
                <button type="button" onClick={() => setIsEditing(true)}
                  className="px-4 py-2 bg-white text-[#A3B18A] rounded-lg text-sm font-medium hover:bg-white/90 transition-colors">Edit</button>
              )}
            </div>
          </div>
          <div className="p-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="bg-gradient-to-r from-[#A3B18A] to-[#8FA076] px-6 py-4 flex items-center space-x-3">
        ...
        {isEditing && (
          <div className="flex justify-end space-x-4">
            ...
            <button type="submit" disabled={saving}
              className={`px-8 py-3 rounded-lg text-white font-medium transition-colors ${saving ? 'bg-gray-400 cursor-not-allowed' : 'bg-[#A3B18A] hover:bg-[#8FA076]'}`}>
              {saving ? (
